...
def quickie(arr): #sort an immutables list
    if len(arr) < 2:
        return arr
    left, right = [], []
    for i in range(1, len(arr)):
        if arr[i] <= arr[0]:
            ...
            right.append(arr[i])
    return quickie(left) + [arr[0]] + quickie(right)
            
#print quickie([1,5, 7, 3, 7,2,7, 43, 65,32])
    

def quicksort(urls, ranks):  #receives a list of urls and global ranks dict, returns sorted urls
    if len(urls) < 2:
        return urls
    left, right = [], []
    for i in range(1, len(urls)):
        if ranks[urls[i]] > ranks[urls[0]]:
            left.append(urls[i])
        else:
            right.append(urls[i])
    return quicksort(left, ranks) + [urls[0]] + quicksort(right, ranks)


def ordered_search(index, ranks, keyword):
    if keyword not in index:
        return None
    return quicksort(index[keyword], ranks)
