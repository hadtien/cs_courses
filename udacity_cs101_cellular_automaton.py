...
words = {1: '...', 2: '..x', 4: '.x.', 8: '.xx', 16: 'x..', 32: 'x.x', 64: 'xx.', 128: 'xxx'}
nums = [128, 64, 32, 16, 8, 4, 2, 1]

def create_lookup_table(pattern): #creates a lookup dict for given pattern
    table = {}
    for i in nums:
        if pattern >= i:
            table[words[i]] = 'x'
...
def mutate(src, table): # mutates one generation of source according to the lookup table
    if len(src) == 1:
        return table[src + src + src]
    res, word = '', ''
    for i in range(len(src)): #iterate over string
        if i == 0: #first letter
            word = src[-1] + src[0] + src[1]
        elif i == len(src) - 1: #last letter
            word = src[-2] + src[-1] + src[0]
        else:
            word = src[i-1] + src[i] + src[i+1]
        res += table[word]
    return res

def cellular_automaton(src, pattern, gen):
    table = create_lookup_table(pattern)
    for i in range(gen):
        src = mutate(src, table)
    return src
