...
def formula(coefs):
    ...
    for i, coef in enumerate(coefs):
        ...
        term = ''
        if i > 1 : 
           if coef != 1: term = str(coef) + ' * x**' + str(i)
           else: term = 'x**' + str(i)
        if i == 1: 
           if coef != 1: term = str(coef) + ' * x'
           else: term = 'x'
        if i == 0: term = str(coef)
        res.append(term)
    res.reverse()
    return ' + '.join(res) 
...
def is_poly(x):
    ...
    return hasattr(x, 'coefs') and isinstance(x.coefs, tuple)

def superzip(t1, t2): # zip(t1, t2) with paddings of zeroes where one tuple is longa
    if len(t1) == len(t2) : return zip(t1, t2)
    if len(t1) < len(t2):  t1, t2 = t2, t1 #t1 is the longer tuple
    return zip(t1, tuple(list(t2) + [0] * (len(t1) - len(t2))))

def add(p1, p2):
    "Return a new polynomial which is the sum of polynomials p1 and p2."
    return poly(tuple([c1 + c2 for c1, c2 in superzip(p1.coefs, p2.coefs)]))
...
def mul(p1, p2):
    ...
    res = [0] * (len(p1.coefs) + len(p2.coefs) - 1)
...
def deriv(p):
    ...
    res_coefs = []
    for i, coef in enumerate(p.coefs): 
        if i == 0: continue #discard the x^0 coef
        res_coefs.append(coef * i)
    return poly(tuple(res_coefs))
