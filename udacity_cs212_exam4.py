...
def shortest_path_search(start, successors, is_goal):
    ...
    print "SPC: explored", len(explored), "states"
    return []
...
def get_loc(state, obj): # returns a tuple of all obj locations
    try:
        return dict(state)[obj]
    except KeyError:
        return None

def get_cell(state, ind): # returns the obj in given ind
    for obj, locs in state:
        if ind in locs: return obj
    return '.'
            
def possible_moves(state, obj): # returns a list of all poss moves disregarding other objects
    # now it disregards cars but takes wall into consideration
    coords = sorted(list(get_loc(state, obj)))
    walls = list(get_loc(state, '|'))
    goal = get_loc(state, '@')[0]
    if goal in walls: walls.remove(goal)
    res = []
    shift = coords[-1] - coords[-2] # either 1 or N
    i = shift
    while coords[-1] + i not in walls: #check the move to the right or down
        if get_cell(state, coords[-1] + i) not in '.@':
            break
        res.append(i)
        i += shift
    i = -shift
    while coords[0] + i not in walls: #check the move to the left or up
        if get_cell(state, coords[0] + i) not in '.@':
            break
        res.append(i)
        i -= shift
    return res
...
def is_goal(state):
    return get_loc(state, '@')[0] in get_loc(state, '*')

def update_state(state, obj, move): #returns updated state
    state_dict = dict(state)
    locs = [elem + move for elem in list(state_dict[obj])]
    state_dict[obj] = tuple(locs)
...
def successors(state):
    ...
    A state is a tuple of tuples(obj, (loc1, ..., loc_n).
    An action is a tuple (obj, shift), i.e. ('A', -3) - go left or ('*', N) - go down
    """
    res = {}
    for obj in state:
        if obj[0] in '@|': continue #dont move walls!
        for move in possible_moves(state, obj[0]):
            #print "successors: updating move", (obj[0], move)
            res[update_state(state, obj[0], move)] = (obj[0], move)
    #print "successors found new
    return res
...
def grid(cars, N=N):
    ...
    gr = list(cars)
    gr.append(('@', (31,))) #BUILT IN CONST!!!!!
    walls = (range(N) + range(N*N - N, N*N) + [i * N for i in range(1, N - 1)] +
             [i * N + N - 1 for i in range(1, N - 1)])

    if 31 in walls: walls.remove(31)
    gr.append(('|',tuple(sorted(walls)))) 
    return tuple(gr)
...
show(puzzle1)
#print possible_moves(puzzle1, 'Y')
#print get_cell(puzzle1, 26)
#successors(puzzle1)
#show(update_state(puzzle1, 'A', -2))
