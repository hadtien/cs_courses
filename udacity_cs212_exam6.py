...
def rateword(src1, src2, portm):
    #returns the rate for portmanteau made from src1 and src2
    if portm.find(src2) < 0:
        return 0
    start_len = portm.index(src2) 
    end_len = len(portm) - len(src1)
    mid_len = len(src2) - end_len
    ideal_quarter, ideal_half = len(portm) / 4, len(portm) / 2
      
    return len(portm) - abs(start_len - ideal_quarter) - \
            abs(mid_len - ideal_half) - abs(end_len - ideal_quarter)

def best_natasha(src1, src2):
    # returs a tuple of (src1, src2, portmanteau, score).
    # if no natash exists, then portmanteau is '' and score iz 0

    best_score, natasha = 0, ''
    for i in range(len(src1) - 1, 0, -1):
        if src2.find(src1[i:]) == 0:
            natasha = src1 + src2.replace(src1[i:], '', 1)
            temp_score = rateword(src1, src2, natasha)
            if temp_score > best_score: best_score = temp_score
    return (natasha, best_score) #default
    

def natalie(words):
    "Find the best Portmanteau word formed from any two of the list of words."
    best_score, best_port = 0, ''
    for w1 in words:
        for w2 in words:
            if w1 == w2: continue
            w1_and_w2, score = best_natasha(w1, w2)
            if score > best_score:
                best_score, best_port = score, w1_and_w2
